...
def get_or_create_dataset_id(engine: Engine, name: str) -> int:
    with engine.begin() as conn:
        ...
        if row:
            return int(row[0])
        row = conn.execute(text("INSERT INTO datasets(name) VALUES (:n) RETURNING id"), {"n": name}).fetchone()
        return int(row[0])
...
def replace_dataset_with_df(engine: Engine, dataset_id: int, df: pd.DataFrame) -> None:
    # Expect df columns: supplier, product, details, website, phone, login_info
    df = df.copy()
    for c in CANON_COLS:
...
def add_record(engine: Engine, dataset_id: int, supplier: str, product: str,
               details: str="", website: str="", phone: str="", login_info: str="") -> None:
    with engine.begin() as conn:
        conn.execute(
            text("""
            INSERT INTO records(dataset_id, supplier, product, details, website, phone, login_info)
